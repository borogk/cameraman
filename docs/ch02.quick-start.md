...
After loading the mod, we can start doing some camerawork.

## How to make a simple camera

First, if you loaded the mod correctly using `cm-editor.exe`, you may notice a few cheats are enabled automatically:
- you can fly and pass through obstacles (`noclip2` cheat is activated)
- enemies ignore you (`notarget` cheat is activated)
- you're invincible (`god` cheat is activated)
