...
Often times the **Editor** module is enough by itself. You may set up camera profiles and play them - all in a single tool.
...
Then, make a camera profile, open the editor menu and hit `Export`:

![](img/cman-menu-export.png)

Then, exit ZDoom. Note the `saved export-xxxx.cman` line in console output - that's your camera profile.
...
Now we finally get to use `cm-player.exe`. It works much the same as `cm-editor.exe`, such as passes arguments to ZDoom, 
but it also has a special `load` command:
```shell
.\cm-player.exe load export-xxxx.cman <args>
```

Args to the right of `load export-xxxx.cman` will be passed as-is to ZDoom executable.

Example: 
```shell
.\cm-player.exe load export-0001.cman -iwad doom2 -file sunlust.wad -warp 3
```
...
```shell
.\cm-editor.exe load export-xxxx.cman <args>
```
...
There's a small caveat: ZDoom remembers which mods you have when the game is saved. And it won't let you load a savefile unless the same exact mods are loaded too!

That's why if you want to use saves and loads with the **Player** module, start ZDoom like this:
```shell
.\cm-player.exe <args>
```
...
Then, you can simply load both the camera profile and the savefile like this:
```shell
.\cm-player.exe load export-xxxx.cman -loadgame save.zds <other args>
```
