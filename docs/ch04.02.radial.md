...
> [!WARNING]
> Make sure you've read [Chapter 4.1: Linear mode](ch04.01.linear.md) first!
> It explains a few base concepts which won't be explained here.
...
**RA0** and **RA1** define the start and end angles. By default `RA0 = 0.0` and `RA1 = 1.0`, which makes a full circle, 
...
Keep in mind, while drawing a radial path interactively, you have limited control over the following parameters:
- **RA0** will always reset to `0.0`
- **RA1** will always reset to `1.0`
- **CX0, CY0** will always be equal to **CX1, CXY**

At this time, the **Cameraman Editor** only allows adjusting these parameters manually.
