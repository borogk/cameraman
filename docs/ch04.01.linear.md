...
> [!NOTE]
> This section not only explains how the **Linear** mode works, but also the meaning of nearly all the extra parameters.
> It's crucial you understand the linear mode first, learning other modes builds on this knowledge!
...
> [!NOTE]
> Play mode does not affect the game HUD, adjust it yourself if needed (using `[+]` and `[-]` keys by default).
...
They are stored in [CVARs](https://zdoom.org/wiki/CVARs) and control the following for **ANY** kind of the path (not only **Linear**):
- Path type and its defining coordinates
- Camera speed
- Camera angle and pitch

Let's start with coordinates. For a **Linear** path they are stored in the following parameters:
- **X0, Y0, Z0** for the starting point
- **X1, Y1, Z1** for the ending point
...
> [!WARNING]
> Be careful with using the mouse on these sliders, it's too sensitive! Use the arrow keys instead.

Also, you can input parameters by setting corresponding CVARs directly in the console.
All parameter names are in lower case and have `cman_` prefix:

![](img/cman-console-cvars.png)

> [!NOTE]
> Full reference of CVARs can be found in [Appendix A: List of CVARs](ap01.cvars.md)
...
> [!NOTE]
> The term **"Angle"** in ZDoom refers to horizontal rotation only.
> Vertical rotation is called **"Pitch"** and will be discussed further below.
...
- **A0** is the angle at the start
- **A1** is the angle at the end
...
- **Relative**: `0.0` value points at the direction of the camera path
...
- In **Distance** mode, the **Speed** value controls the number of units per gametic (1/35 sec)
- In **Time** mode, the **Speed** value controls the number of gametics required to travel the whole path
...
- **Relative** angle mode is better if the viewport is following a path.
...
- You can use Angle and Pitch status UI for reference:
